...
label2=paste("prueba1_cluster_prdm9_N1000_t1M_r",sim,"_p2_X3_DNull_C",C,"_U3.",sep="")

setwd(paste("~/Documents/Projects/PZIFE/Prdm9Nicolas/dataFromCluster/TR_2019_02_14/",sep=""))

m <- rbind(c(1,1,1), c(2,2,2),c(3,3,3))
layout(m,heights = c(3,3,3))

par(mar = c(0.5,5, 0.5, 0.5))

nrow(a)
a=read.table(paste(label,"allelic_class_freq_history",sep=""))
a=a[starTime:endTime,]
d=read.table(paste(label,"allelic_class_history",sep=""))
...
a=read.table(paste(label,"zn_finger_freq_history",sep=""))
a=a[starTime:endTime,]
d=read.table(paste(label,"zn_finger_history",sep=""))
d=d[starTime:endTime,]
