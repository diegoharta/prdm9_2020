...
label2=paste("prueba1_cluster_prdm9_N1000__r",sim,"_N1000_p",p,"_X",X,"_DNull_C",C,"_U",U,".",sep="")

setwd(paste("~/Documents/Projects/PZIFE/Prdm9Nicolas/dataFromCluster/TR_2019_03_01/CU/data/",sep=""))

nombrePlot=paste("prdm9_freqs_labels_C",C,"_U",U,"_c.pdf",sep="")
pdf(nombrePlot,height = 6, width=8)
...
a=read.table(paste(label2,"zf_freq_history",sep=""))
a=a[starTime:endTime,]
d=read.table(paste(label2,"zf_history",sep=""))
d=d[starTime:endTime,]
...
plot(-100,-100,ylim=c(0,2),xlim=c(init,nrow(a)),ylab="Allelic Class Frequencies",xlab="",cex.lab=1.4,sub=label2)
count=1
abline(h=1,col=1)
for(i in 1:length(maxim)){
  if(maxim[i] > umbral){
