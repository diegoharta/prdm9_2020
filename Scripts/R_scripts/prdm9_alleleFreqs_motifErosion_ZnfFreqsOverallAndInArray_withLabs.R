...
label2="prueba1_cluster_prdm9_N1000__r1_N1000_pNull_X3_DNull_C-1_U4."
...
layout(m,heights = c(3,3,3,4))

par(mar = c(0.5,5, 0.5, 0.5))

nrow(a)
a=read.table(paste(label2,"allelic_class_freq_history",sep=""))
a=a[starTime:endTime,]
d=read.table(paste(label2,"allelic_class_history",sep=""))
d=d[starTime:endTime,]
act=read.table(paste(label2,"alelic_class_act_history",sep=""))
...
times=mat.or.vec(1,ncol(newA))
counts=mat.or.vec(1,ncol(newA))
moment=mat.or.vec(1,ncol(newA))
sumaCols=mat.or.vec(1,ncol(newA))

for(i in 1:(ncol(newA))){
...
a_zf=read.table(paste(label2,"zf_freq_history",sep=""))
a_zf=a_zf[starTime:endTime,]
d_zf=read.table(paste(label2,"zf_history",sep=""))
d_zf=d_zf[starTime:endTime,]
...
init=0
par(mar = c(0.5,5, 0.5, 0.5),xaxs="i")
plot(-100,-100,ylim=c(0,2),xlim=c(init,nrow(a_zf)),ylab="Allelic Class Frequencies",xlab="",cex.lab=1.4,xaxt="n")
count=1
abline(h=1,col=1)
for(i in 1:length(maxim_zf)){
  if(maxim_zf[i] > umbral){
    selected[count]=i #guarda las trayectorias que pasan el threshold
    lines(newA_Znf[,i],col=col_vector[count*colorExtension])
    count=count+1
  }
}
...
for(jj in 1:(length(sortUq_zf))){
  if(maxim_zf[jj]>umbral){
    ...
    text(lugar,(maxim_zf[jj]+1),textoA,cex=1.2,col=col_vector[countPosition*colorExtension])
...
a_zf=read.table(paste(label2,"zf_freq_history_bind",sep=""))
a_zf=a_zf[starTime:endTime,]
d_zf=read.table(paste(label2,"zf_history_bind",sep=""))
d_zf=d_zf[starTime:endTime,]
...
plot(-100,-100,ylim=c(0,2),xlim=c(init,nrow(a_zf)),ylab="Allelic Class Frequencies",xlab="",cex.lab=1.4,sub=label2)
count=1
abline(h=1,col=1)
for(i in 1:length(maxim_zf)){
  if(maxim_zf[i] > umbral){
